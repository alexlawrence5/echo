...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer('wpe', self._init_sinusoidal_pe())
    ...
    def _init_sinusoidal_pe(self):
        pe = torch.zeros(self.config.block_size, self.config.n_embd)
        position = torch.arange(0, self.config.block_size).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, self.config.n_embd, 2) * 
                             -(torch.log(torch.tensor(10000.0)) / self.config.n_embd))
